...
{   
    ...
    if (WSAStartup(MAKEWORD(2, 2), &ws) != 0) {
        cout << "Error in intializing winsock" << endl;
        return 0;
    }
    cout << "WSA variables initialised successfully" << endl;
    ...
    if (sSocket == INVALID_SOCKET) {
        cout << "Error in creating socking" << endl;
        return 0;
    }
    ...
    int sbind = bind(sSocket, (sockaddr*)&svr, sizeof(svr));
    ...
    SOCKET sock_client = accept(sSocket, (sockaddr*)&client_addr, &len_client);
    if (sock_client == INVALID_SOCKET) {
        cout << "Error in creating socking. TERMINATING " << endl;
        return 0;
    }
	...
	int originalLen = strlen((const char*)msg);

	string len = to_string(originalLen);
	cout << len << endl;
	...
	for (int i = 0; i < paddedLen; i++) {
		if (i >= originalLen) paddedmsg[i] = 0;
		else paddedmsg[i] = msg[i];
	}
	...
	for (int i = 0; i < paddedLen; i += 16) {
		AESEncrypt(paddedmsg + i, expandedKey, encryptedMessage + i);
	}
	...
	send(sock_client,(const char*)encryptedMessage, paddedLen, 0);
	send(sock_client, len.c_str() , len.length()+1 , 0);
